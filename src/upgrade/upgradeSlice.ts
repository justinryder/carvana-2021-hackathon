...
export const upgradeSlice = createSlice({
  ...
  reducers: {
    ...
    updatePacket: (state, action: PayloadAction<Packet>) => ({
      ...state,
      packets: state.packets.map(packet => packet.id === action.payload.id ? {
        ...packet,
        ...action.payload,
      } : packet)
    }),
    setWindowBounds: (state, action: PayloadAction<Bounds>) => {
      if (state.windowBounds.width === action.payload.width && state.windowBounds.height === action.payload.height) {
        return;
      }
...
export const { purchase, completePacket, openEnvelope, generateWorkForDay, setWindowBounds, updatePacket } =
...
export const _getBuckets = createSelector([
  getState,
], state => state.buckets);
...
export const _getPackets = createSelector([
  getState,
], state => state.packets);
...
export const getPackets = createSelector([
  _getPackets,
  getWindowBounds,
], (packets, windowBounds) => packets.map(packet => ({
  ...packet,
  isInWindow: boxIntersection(windowBounds, packet.bounds),
})));

export const getDraggedPacket = createSelector([
  getPackets,
], packets => packets.find(packet => packet.isDragging));
...
export const __getBuckets = createSelector([
  ...
  const spacing = (windowBounds.width - bucketCount * BUCKET_WIDTH) / (bucketCount + 1);
...
export const getBuckets = createSelector([
  __getBuckets,
  getDraggedPacket,
], (buckets, packet) => packet ? buckets.map(bucket => {

  const isPacketInBucket = boxIntersection(bucket.bounds, packet.bounds);

  const packetMatch = isPacketInBucket && packet.packetType === bucket.bucketType;
  const packetError = isPacketInBucket && packet.packetType !== bucket.bucketType;
